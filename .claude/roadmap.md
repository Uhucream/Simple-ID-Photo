...
4. 内部設計のリファクタ・内部実装の修正
   ...
   - エラー定義をめんどくさがって fatalError 投げてみたり、適当に丸めたりしてる箇所が多数あるので、それらをしっかりエラー定義する
   - Create / Edit の Container において、init に多くの初期化を書くのをめんどくさがって computed property で宣言しているものが多くあるので修正する
   - Task { @MainActor in } の代わりに MainActor.run を使う
5. パスポートサイズ対応
6. 国際化対応
...
20. Apple Archive 等(フレームワークは**要検討**)を用いて、写真が保存されているディレクトリを圧縮する (たった2件作っただけで50MBも消費するのはまずい)
